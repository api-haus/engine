...
- **On the node itself.** The offending node turns red — a red border, and a red ✕ badge at the head of its title bar. Hover the node for the compiler's message. A node the compiler only *warns* about gets the same treatment in amber, with a warning triangle.

//! Validates generated material shaders through naga, the same front end wgpu
//! compiles them with for real.
//!
//! Material codegen runs at editor time and hands its WGSL to Bevy, which
//! compiles it on the GPU path. Nothing between those two points used to check
//! that the generated code is valid — a bad graph did not error, it rendered
//! wrong (or as the fallback) with at most one line in the log. That is a bad
//! failure mode for a shader nobody can eyeball without a GPU, so the output
//! is parsed and validated here instead, by the same front end (naga 29, the
//! version wgpu 29 links) that will compile it for real.
//!
//! One validator, three consumers: the test corpus (`#[cfg(test)]` below),
//! the save path (`precompiled::save_compiled`), and the material editor's
//! diagnostics panel. All three report the same strings from this module so
//! the panel, CI and a save-time refusal can never drift apart.
//!
//! What this catches: syntax errors, type errors, bad swizzles, wrong argument
//! counts, undeclared identifiers in *our* generated code. What it can't
//! catch: anything about the Bevy-side declarations, because of the stubbing
//! below — and a mismatch between the WGSL's bindings and the Rust pipeline's
//! bind group layout, since only the driver sees both.
//!
//! # The stub caveat (carried forward from `foliage/shader_test.rs`)
//!
//! The generated fragment shader begins with Bevy preprocessor directives
//! (`#import bevy_pbr::pbr_fragment::pbr_input_from_standard_material`, …)
//! that naga does not speak, so the import lines are stripped and a
//! hand-written stub of the types and functions the shader uses is prepended
//! in their place (`FRAGMENT_STUB` / `VERTEX_STUB` below). Field names are
//! copied from bevy_pbr 0.19.1's `forward_io.wgsl` / `pbr_types.wgsl` /
//! `pbr_functions.wgsl` — if Bevy renames one, the real shader breaks and
//! this stub happily keeps compiling, so treat this as a check on *our* code
//! rather than on the import. Keep the whole stub set in this one module so
//! there is a single thing to update when Bevy moves.

use naga::valid::{Capabilities, ValidationFlags, Validator};

use super::codegen::CompileResult;

/// One validation failure, with naga's codespan-rendered message (source
/// snippet + carets) preserved — that rendering is what makes the message
/// usable in an editor panel or CI log, so it is built here, once, and every
        ...
        Ok(module) => {
            ...
            if let Err(err) = validator.validate(&module) {
                errors.push(ValidationError {
                    shader: stage,
                    defines: label(),
                    // ValidationError's codespan rendering needs the source it
                    // was produced against; `emit_to_string` on the parse error
                    // type does not exist for validation errors, so fall back
                    // to the Debug dump, which still names the failing rule.
                    message: format!("{err:?}"),
                });
            }
        }
...
// ── Tests ───────────────────────────────────────────────────────────────────
//
// Two corpora, complementary:
//
//   * `assets/materials/*.material` — 65 real shipped graphs. These are what
//     users actually open; a regression that breaks any of them is a
//     regression a user sees.
//   * one synthetic graph per entry in `nodes::ALL_NODES` — pins every node
//     type's codegen down, including the 146 node types no shipped asset
//     happens to use.
//
// Both compile through the real `codegen::compile` and validate through the
// real naga. A failure here means wgpu would have rejected the shader at
// pipeline-creation time on a user's machine.

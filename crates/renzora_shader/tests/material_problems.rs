...
impl Fixture {
    ...
    /// Write the graph, compiling its `.wgsl` the way an editor save does.
    fn write(&mut self, code: &str) {
        ...
        let (json, _) = renzora_shader::material::precompiled::save_compiled_and_serialize(
            &mut graph, &self.dir, &fs_path,
        )
        .unwrap();
        std::fs::write(&fs_path, json).unwrap();
    }

    /// Write the graph JSON with no precompiled `.wgsl`, so the resolver runs
    /// its legacy codegen path — the one that owns codegen warnings.

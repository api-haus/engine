...
fn build(commands: &mut Commands, fonts: &EmberFonts) -> Entity {
    ...
    // Diagnostics strip along the bottom: codegen errors, naga validation
    // rejections, warnings and refused connections — everything
    // `apply_compile_result`, the connect gate and the save path funnel into
    // `MaterialEditorState`. Hidden while both channels are empty.
    let diag = commands
        .spawn((
...
/// Errors first (red), then warnings (amber) — one row each, monospace, in
/// the shape of the shader editor's compiler log.
        ...
        for edit in edits {
            match edit {
                ...
                GraphEdit::Connect { from_node, from_pin, to_node, to_pin } => {
                    // Hard type gate. `PinType::compatible` is exactly the set
                    // of pairs codegen's `cast_expr` can honor, but it was only
                    // ever used as a sort key — so the editor happily wired a
                    // `bool` into a float pin and the shader failed at draw
                    // time. Reject here and say why; the message sits in
                    // compile_errors until the next successful recompile.
